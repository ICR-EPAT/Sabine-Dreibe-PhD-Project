...
```{r progression table, include=TRUE}
...
pubmed_latest_entries$`number of arms ADDED` <- as.numeric(as.character(pubmed_latest_entries$`number of arms ADDED`))
  ...
  progression_table <- pubmed_latest_entries %>%
    tbl_summary(
      ...
      statistic = list(
        all_categorical() ~ "{n} ({p}%)",
        all_continuous() ~ "{median} [{p25}, {p75}]; range: {min}–{max}",# Use median, IQR, range for continuous
        `number of arms DROPPED` ~ "{median} [{p25}, {p75}]; range: {min}–{max}"# Use median, IQR, range for continuous
      ),
      digits = list(
        all_categorical() ~ c(0, 1),
        all_continuous() ~ 1,  # Display one decimal for the continuous variable
        `number of arms DROPPED` ~ 1
      ),
      missing = "no"
    ) %>%
